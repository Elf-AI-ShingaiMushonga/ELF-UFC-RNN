...
def coalesce_int(primary: Optional[int], fallback: Optional[int]) -> Optional[int]:
    return primary if primary is not None else fallback


def merged_fight_stats(stub: FightStub, details: FightDetails) -> dict[str, Optional[int]]:
    return {
        "kd_1": coalesce_int(details.kd_1, stub.kd_1),
        "kd_2": coalesce_int(details.kd_2, stub.kd_2),
        "sig_str_1_landed": coalesce_int(details.sig_str_1_landed, stub.sig_str_1_landed),
        "sig_str_1_attempted": coalesce_int(details.sig_str_1_attempted, stub.sig_str_1_attempted),
        "sig_str_2_landed": coalesce_int(details.sig_str_2_landed, stub.sig_str_2_landed),
        "sig_str_2_attempted": coalesce_int(details.sig_str_2_attempted, stub.sig_str_2_attempted),
        "td_1_landed": coalesce_int(details.td_1_landed, stub.td_1_landed),
        "td_1_attempted": coalesce_int(details.td_1_attempted, stub.td_1_attempted),
        "td_2_landed": coalesce_int(details.td_2_landed, stub.td_2_landed),
        "td_2_attempted": coalesce_int(details.td_2_attempted, stub.td_2_attempted),
        "sub_1": coalesce_int(details.sub_1, stub.sub_1),
        "sub_2": coalesce_int(details.sub_2, stub.sub_2),
        "ctrl_seconds_1": coalesce_int(details.ctrl_seconds_1, stub.ctrl_seconds_1),
        "ctrl_seconds_2": coalesce_int(details.ctrl_seconds_2, stub.ctrl_seconds_2),
    }
...
class RawCheckpointStore:
    ...
    def insert_raw_fight(self, row: dict[str, Any]) -> None:
        ...
        self.conn.execute(
            f"INSERT OR IGNORE INTO raw_fights ({columns}) VALUES ({placeholders})",
            values,
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--max-fights", type=int, default=None, help="Max new fights to process this run.")
    parser.add_argument(
        "--start-date",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Commit every N new fights.",
    )
    parser.add_argument(
        "--stop-on-fight-error",
        action="store_true",
