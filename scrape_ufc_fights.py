...
VALIDATION_REQUIRED_NON_MISSING_COLUMNS = [
    "fighter_1_sig_str_accuracy_pre",
    "fighter_2_sig_str_accuracy_pre",
    "fighter_1_sig_str_defense_pre",
    "fighter_2_sig_str_defense_pre",
...
VALIDATION_EXPECT_NONZERO_COLUMNS = [
    "fighter_1_sig_str_landed_per_min_pre",
    "fighter_2_sig_str_landed_per_min_pre",
    "fighter_1_td_landed_per_15_pre",
    "fighter_2_td_landed_per_15_pre",
...
class FightDetails:
    ...
    kd_1: Optional[int] = None
    kd_2: Optional[int] = None
    sig_str_1_landed: Optional[int] = None
    sig_str_1_attempted: Optional[int] = None
    sig_str_2_landed: Optional[int] = None
    sig_str_2_attempted: Optional[int] = None
    td_1_landed: Optional[int] = None
    td_1_attempted: Optional[int] = None
    td_2_landed: Optional[int] = None
    td_2_attempted: Optional[int] = None
    sub_1: Optional[int] = None
    sub_2: Optional[int] = None
    ctrl_seconds_1: Optional[int] = None
    ctrl_seconds_2: Optional[int] = None
...
def is_missing_csv_value(value: Optional[str]) -> bool:
...
def validate_exported_csv(csv_path: Path) -> list[str]:
    issues: list[str] = []
    ...
    with csv_path.open(newline="", encoding="utf-8") as handle:
        ...
        if missing_columns:
            issues.append(f"CSV is missing expected columns: {', '.join(sorted(missing_columns))}")
            return issues

        total_rows = 0
        present_counts = {column: 0 for column in VALIDATION_REQUIRED_NON_MISSING_COLUMNS}
        nonzero_counts = {
            column: 0
            for column in (VALIDATION_EXPECT_NONZERO_COLUMNS + VALIDATION_LOSS_COLUMNS)
        }

        for row in reader:
            total_rows += 1
            for column in VALIDATION_REQUIRED_NON_MISSING_COLUMNS:
                value = row.get(column)
                if not is_missing_csv_value(value):
    ...
    if total_rows == 0:
        return [f"CSV has zero rows: {csv_path}"]

    for column, present in present_counts.items():
        if present == 0:
            issues.append(f"{column} is missing in every row (0/{total_rows} present).")

    for column in VALIDATION_EXPECT_NONZERO_COLUMNS:
        if nonzero_counts[column] == 0:
            issues.append(f"{column} has no non-zero values across {total_rows} rows.")

    for column in VALIDATION_LOSS_COLUMNS:
        if nonzero_counts[column] == 0:
            issues.append(f"{column} never increments above zero across {total_rows} rows.")

    return issues


def log_export_validation(csv_path: Path) -> list[str]:
    issues = validate_exported_csv(csv_path)
    if not issues:
        logging.info("Post-export validation passed for %s", csv_path)
        return []
    logging.warning(
        "Post-export validation found %s issue(s) in %s",
        len(issues),
        csv_path,
    )
    for issue in issues:
        logging.warning("Validation issue: %s", issue)
    return issues
...
def normalize_status(raw_status: str) -> str:
    ...
    if value in {"W", "WIN"} or "WIN" in value:
        return "W"
    if value in {"L", "LOSS"} or "LOSS" in value:
        return "L"
    if value in {"D", "DRAW"} or "DRAW" in value:
        return "D"
    if value in {"NC", "N/C", "NO CONTEST"} or "NO CONTEST" in value:
        return "NC"
    return ""
...
def parse_landed_attempted_or_landed(value: str) -> tuple[Optional[int], Optional[int]]:
    landed, attempted = parse_landed_attempted(value)
    if landed is None and attempted is None:
        landed = parse_int_from_text(value)
    return landed, attempted


def infer_fighter_statuses(status_cell: Optional[Any], method_text: str) -> tuple[str, str]:
    statuses = [normalize_status(value) for value in td_values(status_cell)]
    statuses = [status for status in statuses if status]
    fighter_1_status = statuses[0] if len(statuses) > 0 else ""
    fighter_2_status = statuses[1] if len(statuses) > 1 else ""

    method_lower = clean_text(method_text).lower()
    if not fighter_1_status and not fighter_2_status:
        if "draw" in method_lower:
            return "D", "D"
        if "no contest" in method_lower or method_lower in {"nc", "n/c"}:
            return "NC", "NC"
        if method_lower:
            return "W", "L"
        return "", ""

    if fighter_1_status == "W" and not fighter_2_status:
        fighter_2_status = "L"
    elif fighter_2_status == "W" and not fighter_1_status:
        fighter_1_status = "L"
    elif fighter_1_status == "L" and not fighter_2_status:
        fighter_2_status = "W"
    elif fighter_2_status == "L" and not fighter_1_status:
        fighter_1_status = "W"
    elif fighter_1_status in {"D", "NC"} and not fighter_2_status:
        fighter_2_status = fighter_1_status
    elif fighter_2_status in {"D", "NC"} and not fighter_1_status:
        fighter_1_status = fighter_2_status

    return fighter_1_status, fighter_2_status
...
def extract_fight_totals_stats_from_soup(soup: BeautifulSoup) -> dict[str, Optional[int]]:
    ...
    for table in soup.select("table.b-fight-details__table"):
        ...
        if not headers:
            continue
        if "fighter" not in headers:
            continue
        if "ctrl" not in headers:
            continue
        if "sigstr" not in headers and "str" not in headers:
            continue
        ...
        for row in rows:
            ...
            for idx, header in enumerate(headers):
                if idx < len(cells) and header not in mapped:
                    mapped[header] = cells[idx]
            fighter_cell = mapped.get("fighter")
            if fighter_cell is None:
                continue
            fighter_links = fighter_cell.select("a[href*='/fighter-details/']")
            if len(fighter_links) < 2:
                continue
            kd_1_raw, kd_2_raw = td_pair_values(mapped.get("kd"))
            sig_cell = mapped.get("sigstr") or mapped.get("str")
            sig_str_1_raw, sig_str_2_raw = td_pair_values(sig_cell)
            td_1_raw, td_2_raw = td_pair_values(mapped.get("td"))
            sub_cell = mapped.get("subatt") or mapped.get("sub")
            sub_1_raw, sub_2_raw = td_pair_values(sub_cell)
            ctrl_1_raw, ctrl_2_raw = td_pair_values(mapped.get("ctrl"))
            sig_str_1_landed, sig_str_1_attempted = parse_landed_attempted_or_landed(sig_str_1_raw)
...
def parse_event_fights(client: HttpClient, event: EventMeta) -> list[FightStub]:
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        for h_idx, header in enumerate(headers):
            if h_idx < len(cells) and header not in mapped:
        ...
        sig_str_cell = mapped.get("str") or mapped.get("sigstr")
        ...
        method_text = method_values[0] if method_values else ""
        fighter_1_status, fighter_2_status = infer_fighter_statuses(status_cell, method_text)
...
def parse_fight_details(client: HttpClient, fight_url: str, fallback_weight_class: str) -> FightDetails:
    ...
    totals_stats = extract_fight_totals_stats_from_soup(soup)
    return FightDetails(
        ...
        kd_1=totals_stats.get("kd_1"),
        kd_2=totals_stats.get("kd_2"),
        sig_str_1_landed=totals_stats.get("sig_str_1_landed"),
        sig_str_1_attempted=totals_stats.get("sig_str_1_attempted"),
        sig_str_2_landed=totals_stats.get("sig_str_2_landed"),
        sig_str_2_attempted=totals_stats.get("sig_str_2_attempted"),
        td_1_landed=totals_stats.get("td_1_landed"),
        td_1_attempted=totals_stats.get("td_1_attempted"),
        td_2_landed=totals_stats.get("td_2_landed"),
        td_2_attempted=totals_stats.get("td_2_attempted"),
        sub_1=totals_stats.get("sub_1"),
        sub_2=totals_stats.get("sub_2"),
        ctrl_seconds_1=totals_stats.get("ctrl_seconds_1"),
        ctrl_seconds_2=totals_stats.get("ctrl_seconds_2"),
    )
    ...
    *,
    ...
    for stub in stubs:
        ...
        try:
            ...
            kd_1 = details.kd_1 if details.kd_1 is not None else stub.kd_1
            kd_2 = details.kd_2 if details.kd_2 is not None else stub.kd_2
            sub_1 = details.sub_1 if details.sub_1 is not None else stub.sub_1
            sub_2 = details.sub_2 if details.sub_2 is not None else stub.sub_2
            ctrl_seconds_1 = (
                details.ctrl_seconds_1
                if details.ctrl_seconds_1 is not None
                else stub.ctrl_seconds_1
            )
            ctrl_seconds_2 = (
                details.ctrl_seconds_2
                if details.ctrl_seconds_2 is not None
                else stub.ctrl_seconds_2
            )
            sig_str_1_landed = (
                details.sig_str_1_landed
                if details.sig_str_1_landed is not None
                else stub.sig_str_1_landed
            )
            sig_str_1_attempted = (
                details.sig_str_1_attempted
                if details.sig_str_1_attempted is not None
                else stub.sig_str_1_attempted
            )
            sig_str_2_landed = (
                details.sig_str_2_landed
                if details.sig_str_2_landed is not None
                else stub.sig_str_2_landed
            )
            sig_str_2_attempted = (
                details.sig_str_2_attempted
                if details.sig_str_2_attempted is not None
                else stub.sig_str_2_attempted
            )
            td_1_landed = details.td_1_landed if details.td_1_landed is not None else stub.td_1_landed
            td_1_attempted = (
                details.td_1_attempted if details.td_1_attempted is not None else stub.td_1_attempted
            )
            td_2_landed = details.td_2_landed if details.td_2_landed is not None else stub.td_2_landed
            td_2_attempted = (
                details.td_2_attempted if details.td_2_attempted is not None else stub.td_2_attempted
            )
            updated_state_1 = apply_result_to_state(
                ...
                knockdowns_for=kd_1,
                sub_attempts_for=sub_1,
                control_seconds_for=ctrl_seconds_1,
                sig_str_landed_for=sig_str_1_landed,
                sig_str_attempted_for=sig_str_1_attempted,
                sig_str_landed_against=sig_str_2_landed,
                sig_str_attempted_against=sig_str_2_attempted,
                td_landed_for=td_1_landed,
                td_attempted_for=td_1_attempted,
                td_landed_against=td_2_landed,
                td_attempted_against=td_2_attempted,
            )
            updated_state_2 = apply_result_to_state(
                ...
                fight_duration_seconds=duration_seconds,
                knockdowns_for=kd_2,
                sub_attempts_for=sub_2,
                control_seconds_for=ctrl_seconds_2,
                sig_str_landed_for=sig_str_2_landed,
                sig_str_attempted_for=sig_str_2_attempted,
                sig_str_landed_against=sig_str_1_landed,
                sig_str_attempted_against=sig_str_1_attempted,
                td_landed_for=td_2_landed,
                td_attempted_for=td_2_attempted,
                td_landed_against=td_1_landed,
                td_attempted_against=td_1_attempted,
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Fail the run if post-export CSV validation finds critical data-quality issues "
            "(for example all-missing accuracy fields)."
        ),
    )
    return parser
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        if args.export_only:
            ...
            issues = log_export_validation(args.output_csv)
            if issues and args.strict_validation:
                logging.error("Strict validation failed after export-only run.")
                return 2
            return 0
        ...
        if not pending_events:
            ...
            issues = log_export_validation(args.output_csv)
            if issues and args.strict_validation:
                logging.error("Strict validation failed after CSV refresh.")
                return 2
            return 0
        ...
        issues = log_export_validation(args.output_csv)
        if issues and args.strict_validation:
            logging.error("Strict validation failed.")
            return 2
        return 0
    finally:
        store.close()
